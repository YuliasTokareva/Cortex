...
- /start — привязка аккаунта Telegram к учётной записи  
- /add_goal <текст> — добавление цели  
- /add_note <текст> — добавление заметки 
- /add_deadline <текст> — добавление дедлайна  
...
- при нажатии "/" — показывается список всех доступных команд "
...
# Установка и запуск 
(перед началом реализации прочитайте инструкцию ПОЛНОСТЬЮ, особенно пункт №5)
...
4. Запуск веб-интерфейса (в Docker):
- docker compose up --build — для запуска проекта
- ctrl + c — для остановки работы контейнеров
- docker compose down — для остановки и удаления работы всех контейнеров проекта 
5. Запуск Telegram-бота (локально, в новой вкладке терминала)
...
6. Запуск тестов (именно в том терминале, где запускали Docker)  
Тесты покрывают:
